...
{
public:
	enum RRType { RRUNDEF = 0, A = 1, NS, MD, MF, CNAME, SOA, MB, MG, MR,RRNULL,WKS, PTR, HINFO, MINFO, MX, TXT,AAAA = 28, CERT = 37, OPT = 41, DHCID = 49, TSIG = 250, AXFR = 252, MAILB = 253, MAILA = 254, TYPESTAR = 255};
	...
	static std::string RRTypeToString(RRType t)
	{
		...
		{	SC(A); SC(NS); SC(MD); SC(CNAME); SC(SOA); SC(MB); SC(MR); SC2(RRNULL, NULL); SC(WKS);
			SC(PTR); SC(MINFO); SC(MX); SC(TXT); SC(AAAA); SC(CERT); SC(OPT); SC(DHCID); SC(TSIG); SC(AXFR); SC(MAILB); SC(MAILA); 
			SC2(TYPESTAR, STAR);
			default: 
				std::stringstream ss;
